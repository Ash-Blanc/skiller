...
**Goal:** Had an idea of creating an advanced ai agent system using agno.com and firecrawl and any other scraping fallback agno tool and supermemory.ai given any X user it creates skills of every single person on X followed by the user and keep them handy given any task it determines which one to use n use it very efficiently to get the work done most amazing way possible basically creating the most generalist and skilled ai agent than anyone in the user's network fr And the skill per person will be created based on their legit n novel high quality X posts
